...
data1 = pd.read_csv("teamRecord.csv")
...
for i in range(30):
    ...
    for fi in first_value:
        ...
        if fi == 'W':
            ...
            loss = loss + 1
    print(data['Team'].values[(i + 1) * 81], win, loss)
    table = data['Team'].values[(i + 1) * 81], win, loss
    a_list.append(table)
    win = 0
    loss = 0
    points = 0
...
d_list = ['Team', 'Win', 'Loss']
file = open('teamRecord.csv', 'w+', newline='\n')
